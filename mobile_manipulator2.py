...
velocity_output = [[] for _ in range(robot.getDOF())] # initialize 5 lists to store 5 DoFs

# Weighting matrix
# W = np.diag([2.0, 2.0, 2.0, 2.0, 2.0])
# W = np.diag([10.0, 15.0, 3.0, 2.0, 1.0])
W = np.diag([1.0, 1.0, 10.0, 15.0, 20.0])
...
def init():
    ...
    # desired_EE_random = np.random.uniform(-2.0, 2.0, (2, 1))
...
def simulate(t):
    ...
    # Append velocity output for this timestep to the global list
    for dof in range(dq.shape[0]):
        velocity_output[dof].append(dq[dof,0])
...
def result_plot(error_evol, velocity_evol, dt):
    ...
    ## Plotting velocity output evolution
    fig2 = plt.figure()
    ax2 = fig2.add_subplot(111, xlim=(0, tfinal))
    for i in range(len(velocity_evol)):
        ax2.plot(tt,velocity_evol[i], label=f"Joint {i+1}")

    ax2.set_title('Velocity Output Evolution')
...
result_plot(error_evol, velocity_output, dt)
